  ...
  const {loading} = useSelector(state=>state.user)
     useEffect(() => {
        ...
        return window.addEventListener("scroll", () => {
          ...
          const skillBox = document.getElementById("homeSkillsBoxId")
          if(skillBox){
            if(window.scrollY > 1000){
              skillBox.style.animationName = "homeSkillsBoxAnimationOn"
            }else{
              skillBox.style.animationName = "homeSkillsBoxAnimationOf"
            }
          }
        })
    }, [])
  ...
  return (
    loading ? <Loader/> : <div className='home'>
        ...
        <div className="homeSkills">
          ...
          <div className="homeSkillsBox" id="homeSkillsBoxId">
            <SiCplusplus/>
            <SiHtml5/>
            <SiCss3/>
            <SiJavascript/>
        ...
        <div className="homeYoutube">
          ...
          <div className="homeYoutubeWrapper">
            {youtubes.map((item) => (
              <YouTubeCard key={item._id} id={item._id} url={item.url} title={item.title} image={item.image.url}/>
            ))}
          </div>
        </div>
    </div>
